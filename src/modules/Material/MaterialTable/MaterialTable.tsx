...
const MaterialTable = () => {
  ...
  const { data, refetch } = useFetchMaterialsQuery({
  ...
  const columns = [
    ...
    },
    {
      ...
      header: () => (
        ...
        >
          Tên chất liệu {getSortIcon("name")}
        </div>
      ),
  ...
  return (
    <div className="p-6 bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
      <div className="mb-6">
        <div className="bg-gradient-to-r from-amber-600 to-amber-700 rounded-xl shadow-lg p-6 mb-6">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-white">Quản lý chất liệu</h1>
            <Button
              onClick={() => router.push(getRouteWithRole('/materials/new'))}
              className="flex items-center gap-2 bg-white hover:bg-gray-100 text-amber-700 font-medium shadow-sm"
            >
              ...
              <TableBody>
                {table.getRowModel().rows?.length ? (
                  table.getRowModel().rows.map((row) => (
                    <TableRow 
                      key={row.id} 
                      className="border-b border-gray-100 hover:bg-gradient-to-r hover:from-amber-50 hover:to-transparent transition-all duration-200"
                    >
