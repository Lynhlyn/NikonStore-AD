...
import { getPositionLabel, getDisplayOrderLabel, POSITION_OPTIONS } from '@/common/utils/bannerPosition';
...
const BannerTable = () => {
  ...
  const [queryStates, setQueryStates] = useQueryStates({
    ...
    position: parseAsInteger.withDefault(-1),
    sort: parseAsString.withDefault('displayOrder'),
    direction: parseAsString.withDefault('asc'),
  });

  const { data, refetch } = useFetchBannersQuery(
    {
      ...
      position: queryStates.position !== -1 ? queryStates.position : undefined,
    },
  ...
  const handlePositionChange = (position: { value: number; label: string }) => {
    setQueryStates((prev) => ({
      ...prev,
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success('Đã xóa thành công banner');
      refetch();
      setDeleteModalOpen(false);
      setSelectedBannerId(null);
    } catch (error: any) {
      toast.error('Đã xảy ra lỗi khi xóa banner: ' + error.message);
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: ({ row }: any) => {
        ...
        return <div className="text-left text-sm text-gray-600">{positionLabel}</div>;
      },
    },
    {
      accessorKey: 'imageUrl',
      header: () => <div className="flex items-center text-left">Hình ảnh</div>,
      cell: ({ row }: any) => {
    ...
    },
    {
      ...
      header: () => (
        ...
        >
          Thứ tự hiển thị {getSortIcon('displayOrder')}
        </div>
      ),
      cell: ({ row }: any) => {
        const displayOrder = row.getValue('displayOrder') as number;
        const displayOrderLabel = getDisplayOrderLabel(displayOrder);
        return (
    ...
    },
    {
      ...
      cell: ({ row }: any) => (
        <div className="text-left">{getStatusDisplay(row.getValue('status'))}</div>
      ),
    },
    {
        ...
        return (
          <div className="flex gap-2 justify-end">
            <Button
              variant="outline"
              size="sm"
              onClick={() => router.push(getRouteWithRole(`/banners/${id}/edit`))}
            >
  ...
  return (
    <div className="p-6 bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
      <div className="mb-6">
        <div className="bg-gradient-to-r from-fuchsia-600 to-fuchsia-700 rounded-xl shadow-lg p-6 mb-6">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-white">Quản lý banner</h1>
            <Button
              onClick={() => router.push(getRouteWithRole('/banners/new'))}
              className="flex items-center gap-2 bg-white hover:bg-gray-100 text-fuchsia-700 font-medium shadow-sm"
            >
      ...
      <div className="mb-5 flex items-center gap-4">
        <div className="w-[220px]">
          <UISingleSelect
            ...
            selected={
              queryStates.status
                ? statusOptions.find((item) => item.value === queryStates.status) ||
                  statusOptions[0]
                : statusOptions[0]
            }
            onChange={(selected) =>
              handleStatusChange(selected as { value: string; label: string })
            }
        ...
        <div className="w-[220px]">
          <UISingleSelect
            ...
            selected={
              positionOptions.find((item) => item.value === queryStates.position) ||
              positionOptions[0]
            }
            onChange={(selected) =>
              handlePositionChange(selected as { value: number; label: string })
            }
            className="rounded-md border border-gray-300"
            size={ESize.M}
            renderSelected={(props) => <UISingleSelect.Selected {...props} />}
              ...
              <TableBody>
                {table.getRowModel().rows?.length ? (
                  table.getRowModel().rows.map((row) => (
                    <TableRow 
                      key={row.id} 
                      className="border-b border-gray-100 hover:bg-gradient-to-r hover:from-fuchsia-50 hover:to-transparent transition-all duration-200"
                    >
