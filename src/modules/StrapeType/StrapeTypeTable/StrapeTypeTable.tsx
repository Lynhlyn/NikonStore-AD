...
import { useFetchStrapTypesQuery, useDeleteStrapTypeMutation } from "@/lib/services/modules/strapTypeService";
...
const StrapeTypeTable = () => {
  const router = useRouter();
  const { getRouteWithRole } = useAppNavigation();
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [selectedStrapTypeId, setSelectedStrapTypeId] = useState<number | null>(null);
  ...
  const { data, refetch } = useFetchStrapTypesQuery({
  ...
  const [deleteStrapType, { isLoading: isDeleting }] = useDeleteStrapTypeMutation();
  ...
  const handleDeleteClick = (strapTypeId: number) => {
    setSelectedStrapTypeId(strapTypeId);
    setDeleteModalOpen(true);
  };

  const handleDelete = async () => {
    if (!selectedStrapTypeId) return;
    try {
      await deleteStrapType(selectedStrapTypeId).unwrap();
      toast.success('Đã xóa thành công loại dây đeo');
      refetch();
      setDeleteModalOpen(false);
      setSelectedStrapTypeId(null);
    } catch (error: any) {
      toast.error('Đã xảy ra lỗi khi xóa loại dây đeo: ' + error.message);
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      header: () => (
        ...
        >
          Tên loại dây đeo {getSortIcon("name")}
        </div>
      ),
        ...
        return (
          <div className="flex gap-2 justify-end">
            <Button
              variant="outline"
              size="sm"
              onClick={() => router.push(getRouteWithRole(`/strap-types/${id}/edit`))}
            >
  ...
  return (
    <div className="py-8 px-6">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-semibold text-gray-900">Quản lý loại dây đeo</h1>
        <Button
          onClick={() => router.push(getRouteWithRole('/strap-types/new'))}
          className="flex items-center gap-2"
        >
          <Plus className="w-4 h-4" />
          <span>Thêm loại dây đeo</span>
        </Button>
      </div>
      ...
      <ConfirmModal
        ...
        title="Xác nhận xóa loại dây đeo"
        message="Bạn có chắc chắn muốn xóa loại dây đeo này không? Hành động này không thể hoàn tác."
        type="danger"
...
export default StrapeTypeTable;
