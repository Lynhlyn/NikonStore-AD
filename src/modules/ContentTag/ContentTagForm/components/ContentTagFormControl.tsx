...
import { useContentTagFormContext } from '@/modules/ContentTag/ContentTagForm/components/useContentTagFormControl';
...
const NameInput = () => {
  const { control, formState: { errors } } = useContentTagFormContext();

  return (
    <UIFormControl>
      <UIFormControl.Label isRequired>Tên tag</UIFormControl.Label>
      <Controller
        control={control}
        name="name"
        render={({ field }) => (
...
const TypeInput = () => {
  ...
  return (
    <UIFormControl>
      <UIFormControl.Label isRequired>Loại tag</UIFormControl.Label>
      <Controller
        control={control}
        name="type"
...
const Button = () => {
  const { isLoading, handleSubmit, onSubmit, router, getRouteWithRole } = useContentTagFormContext();

  return (
    <FormAction
      onSubmit={handleSubmit(onSubmit)}
      isLoading={isLoading}
      onCancel={() => router.push(getRouteWithRole('/content-tags'))}
    />
  );
};

const ContentTagFormControl = {
  NameInput,
  SlugInput,
  TypeInput,
  Button,
