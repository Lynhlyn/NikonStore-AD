...
import { useCapacityFormContext } from '@/modules/Capacity/CapacityForm/components/useCapacityFormControl';

const NameInput = () => {
  const { control, formState: { errors } } = useCapacityFormContext();

  return (
    <UIFormControl>
      <UIFormControl.Label isRequired>Tên dung tích</UIFormControl.Label>
      <Controller
        control={control}
        name="name"
        render={({ field }) => (
...
const Button = () => {
  const { isLoading, handleSubmit, onSubmit, router, getRouteWithRole } = useCapacityFormContext();

  return (
    <FormAction
      onSubmit={handleSubmit(onSubmit)}
      isLoading={isLoading}
      onCancel={() => router.push(getRouteWithRole('/capacities'))}
    />
  );
};

const CapacityFormControl = {
  NameInput,
  StatusInput,
  Button,
};

export { CapacityFormControl };
